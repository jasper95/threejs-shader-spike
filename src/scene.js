...
// const reflection = orginPoints.map(p => [p[0], p[1], p[2]*-1, p[3]*-1, p[4], p[5]]);
// const rawPoints = orginPoints.concat(reflection)
    ...
    faces.forEach(triangles => {
      triangles.forEach(triangle => {
        triangle.vertices.forEach(point => {
          ...
          reflection.push(point.x*-1, point.y, point.z*-1);
        })
      })
    })
    const indices = new Array((vertices.length*2)/3).fill(null).map((_,i) => i)
    const geometry = new THREE.BufferGeometry()
    geometry.setIndex(indices);
    geometry.setAttribute('position', new THREE.Float32BufferAttribute(vertices.concat(reflection), 3));
